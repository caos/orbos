...
func (s *keepaliveDDep) Ensure(remove common.Package, ensure common.Package) error {
	...
	if err := ioutil.WriteFile("/etc/keepalived/keepalived.conf", []byte(strings.ReplaceAll(common.StringPointerValue(ensureCfg), "[ REDACTED ]", s.peerAuth)), 0600); err != nil {
		return err
	}

	if notifyMaster, ok := ensure.Config["notifymaster.sh"]; ok {
		if err := ioutil.WriteFile("/etc/keepalived/notifymaster.sh", []byte(common.StringPointerValue(notifyMaster)), 0777); err != nil {
			return err
		}
	}

	if authCheck, ok := ensure.Config["authcheck.sh"]; ok {
		if err := ioutil.WriteFile("/etc/keepalived/authcheck.sh", []byte(common.StringPointerValue(authCheck)), 0777); err != nil {
			return err
		}
	}
