...
func join(
	...
	case "calico":
		applyNetworkCommand = fmt.Sprintf(`curl https://docs.projectcalico.org/v3.16/manifests/calico.yaml | sed -e "s?192.168.0.0/16?%s?g" | kubectl apply -f -`, desired.Spec.Networking.PodCidr)
	default:
