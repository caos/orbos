...
func join(
	...
	case "calico":
		applyNetworkCommand = fmt.Sprintf(`curl https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml -O && sed -i -e "s?10.244.0.0/16?%s?g" flannel.yaml && kubectl apply -f flannel.yaml`, desired.Spec.Networking.PodCidr)
	default:
