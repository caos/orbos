...
func Reconcile(monitor mntr.Monitor, desiredTree *tree.Tree, gitops bool) core.EnsureFunc {
	return func(k8sClient kubernetes.ClientInt) (err error) {
		...
		if err := kubernetes.EnsureDatabaseArtifacts(monitor, treelabels.MustForAPI(desiredTree, mustDatabaseOperator(&desiredKind.Spec.Version)), k8sClient, desiredKind.Spec.Version, desiredKind.Spec.NodeSelector, desiredKind.Spec.Tolerations, imageRegistry, gitops); err != nil {
