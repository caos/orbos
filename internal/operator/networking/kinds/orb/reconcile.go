...
func Reconcile(monitor mntr.Monitor, apiLabels *labels.API, desiredTree *tree.Tree) core.EnsureFunc {
	return func(k8sClient kubernetes.ClientInt) (err error) {
		...
		if err := kubernetes.EnsureNetworkingArtifacts(monitor, apiLabels, k8sClient, desiredKind.Spec.Version, desiredKind.Spec.NodeSelector, desiredKind.Spec.Tolerations, imageRegistry); err != nil {
